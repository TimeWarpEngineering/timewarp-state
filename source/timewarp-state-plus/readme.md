...
- **PersistentState**
  ...
  - **JSON**: Save and load use `TimeWarpStateOptions.JsonSerializerOptions` (the same options as `Store` and `JsonRequestHandler`). Configure converters there so persisted state round-trips.
- 
